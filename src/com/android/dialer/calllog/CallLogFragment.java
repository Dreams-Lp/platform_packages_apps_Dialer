...
public class CallLogFragment extends ListFragment
    ...
    public void configureScreenFromIntent(Intent newIntent) {
        ...
        if (mScrollToTop) {
            filterScreenWithCallType(newIntent.getIntExtra(
                    KEY_FILTER_SCREEN_BY, -1));
        }
    }
    ...
    private void filterScreenWithCallType(int callType) {
        if (callType == CallLogQueryHandler.CALL_TYPE_ALL) {
            ...
            // We only need the phone call receiver when there is an active call
            // type filter.
            // Not many people may use the filters so don't register the
            // receiver until now .
