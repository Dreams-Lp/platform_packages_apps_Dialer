        ...
        if ((len > 1) && (len < 5) && (input.endsWith("#"))) {
            try {
                ...
                if (TelephonyManager.getDefault().isMultiSimEnabled()) {
                    int subscription = TelephonyManager.getDefault().
        ...
        if ((input.startsWith("**04") || input.startsWith("**05")) && input.endsWith("#")) {
            try {
                if (TelephonyManager.getDefault().isMultiSimEnabled()) {
                    ...
                    subscription = TelephonyManager.getDefault()
                            .getPreferredVoiceSubscription();
                    return ITelephony.Stub.asInterface(ServiceManager.getService("phone"))
                            .handlePinMmiUsingSub(subscription, input);
                } else {
                    return ITelephony.Stub.asInterface(ServiceManager.getService("phone"))
                            .handlePinMmi(input);
                }
        ...
        if (input.equals(MMI_IMEI_DISPLAY)) {
            ...
            if (TelephonyManager.getDefault().isMultiSimEnabled()) {
                ...
                phoneType = ((TelephonyManager)context.getSystemService(
                        Context.TELEPHONY_SERVICE)).getCurrentPhoneType(subscription);
            } else {
                phoneType = ((TelephonyManager)context.getSystemService(
    ...
    private static void showIMEIPanel(Context context, boolean useSystemWindow) {
        ...
        if (TelephonyManager.getDefault().isMultiSimEnabled()) {
            ...
            imeiStr = ((TelephonyManager)context.
                    getSystemService(Context.TELEPHONY_SERVICE)).getDeviceId(subscription);
        } else {
            imeiStr = ((TelephonyManager)context.
